...
class UserLikeService {
  ...
  static const String endPoint = ConstantUrl.likeUrl;
  static const String apiKey = ConstantApi.apiKey;

  Future<UserLike?> like(String id) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        log(response.body);
        return UserLike.fromMap(jsonResponse);
      } else {
        log('Failed to Like. Status code: ${response.statusCode}');
        log(response.body.toString());
        return null;
      }
    } catch (e) {
      log('Error while Liking: ${e.toString()}');
      return null;
    }
  }
}
